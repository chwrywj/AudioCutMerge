...
app.whenReady().then(() => {
    ...
    ipcMain.handle('selectAudioForCut', (event, audioFilePath) => {
        ffmpegClass.audioSupport(audioFilePath).then((checkResult) => {
            ffmpegClass.killAudioSupperCommand();
            let playParams = {};
            playParams.duration = checkResult.duration
            playParams.frameRate = checkResult.frameRate
            if (checkResult.audioCodecSupport && checkResult.audioCodecSupport) {
                playParams.type = "native";
                playParams.audioSource = audioFilePath;
            }
            if (!checkResult.audioCodecSupport || !checkResult.audioCodecSupport) {
                ffmpegClass.audioSourceInfo = { audioSourcePath: audioFilePath, checkResult: checkResult };
                ffmpegClass.createAudioServer();
    
                playParams.type = "stream";
                playParams.audioSource = "http://127.0.0.1:8888?startTime=0";
            }
            mainWindow.webContents.send('audioInfoForCut', playParams);
        }).catch((err) => {
            console.log(err)
            // const options = {
            //     type: 'info',
            //     title: 'Error',
            //     message: "It is not a audio file!",
            //     buttons: ['OK']
            // }
